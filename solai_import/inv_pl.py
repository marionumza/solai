...
class import_transaction:
    ...
    def do_create_move(self):
        account_totals = {'6000 Accounting Fees': 7583,
                          ...
                          '7310 Rubbish Removal': 32.45,
                          '7700 Interest Expense': 25513.76,
                          }
    ...
    def add_inv_debit(self):
        invoices = self.models.execute(self.dbname, self.uid, self.pwd, 'account.move', 'search_read',
                                           [['type', '=', 'out_invoice']])
        for invoice in invoices:
            # print(invoice,"AAAAA",invoice['id'])
            total = 0
            # invoice_line = invoice['id']
            lines = self.models.execute(self.dbname, self.uid, self.pwd, 'account.move.line', 'search_read',
                                [['move_id', '=', invoice['id']]])
            # print( "\n" * 20)
            # print(lines,"a"*20)
            for line in lines:
            ...
            # print('*'*15)
            # print(total, invoice['name'])
            # print('*' * 15)
            linec = self.models.execute_kw(self.dbname, self.uid, self.pwd, 'account.move.line', 'create',
                                           ...
                                           [{
                                               ...
                                               'exclude_from_invoice_tab': True,
                                               'debit': abs(total),#float(invoice and  invoice['ref']),
                                               # 'credit': float(credit),
                                           }], {'context': {'check_move_validity': False}})
    ...
    def do_import_transaction(self, csv_file):
        ...
        for row in csvReader:
            if row['Account']:
                if row['Type'] == 'Invdfoice' and not row['COGS']:
                    # print(row)
                    move = self.models.execute(self.dbname, self.uid, self.pwd, 'account.move', 'search_read',
                                               [['name', '=', row['Num']]])
                    ...
                    if move_id:
                        ref = move and move[0] and move[0]['ref']
                        line_date = datetime.strptime(row['Date'].strip(), '%m/%d/%Y').date()
                        name = row['Name']
                        debit_account_code = row['Account'][0:4]
                        debit_account = self.models.execute(self.dbname, self.uid, self.pwd, 'account.account', 'search_read',
                                                   [['code', '=', debit_account_code]])
                        debit_account_id = debit_account and debit_account[0] and debit_account[0]['id']
                        credit_account_code = row['Account'][0:4]
                        credit_account = self.models.execute(self.dbname, self.uid, self.pwd, 'account.account',
                                                             'search_read', [['code', '=', credit_account_code]])
                        credit_account_id = credit_account and credit_account[0] and credit_account[0]['id']

                        debit = row['Debit'].replace(',', '').strip() or 0
                        credit = row['Credit'].replace(',', '').strip() or 0
                        lined = self.models.execute_kw(self.dbname, self.uid, self.pwd, 'account.move.line', 'create',

                                                    [{
                                                        'move_id': move_id,
                                                        'name': row['Memo'],
                                                        'date': line_date.isoformat(),
                                                        'account_id': debit_account_id,  # Receivables
                                                        #'exclude_from_invoice_tab': True,
                                                        'debit': float(debit),
                                                        'credit':float(credit),
                                                    }], {'context': {'check_move_validity': False}})
                        ...
                        #                                    #'name': row[''],
                        #                                    'date': line_date.isoformat(),
                        #                                    'account_id': credit_account_id,  # Receivables
                        #                                    # 'exclude_from_invoice_tab': True,
                        #                                    'debit': float(ref),
                        #                                    #'credit': float(credit),
                        #                                }], {'context': {'check_move_validity': False}})
                    else:
                        print('Inv not found -->', row['Num'])

                else:
                    #if row['Type'] != 'Invoice':
                    # print(row['Type'], row['Split'], row['Account'])
                    #move = self.models.execute(self.dbname, self.uid, self.pwd, 'account.move', 'search_read',
                                              # [['name', '=', row['Account']]])
                    #if not move:
                    line_date = datetime.strptime(row['Date'].strip(), '%m/%d/%Y').date()
                    debit_account_code = row['Account'][0:4]
                    debit_account = self.models.execute(self.dbname, self.uid, self.pwd, 'account.account',
                                                        'search_read',
                                                        [['code', '=', debit_account_code]])
                    debit_account_id = debit_account and debit_account[0] and debit_account[0]['id']
                    # if not debit_account_id:
                    #      print("Acc missing", row['Account'])
                    #      sxav

                    credit_account_code = row['Split'][0:4]
