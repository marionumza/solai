...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self):
        ...
        quotation_count = SaleOrder.search_count([('state', 'in', ['sent', 'cancel']), '|',
            ('message_partner_ids', 'child_of', [partner.commercial_partner_id.id]),
                                                  ('installer_id', '=', request.env.user.id)
        ])
        order_count = SaleOrder.search_count([('state', 'in', ['sale', 'done']), '|',
            ('message_partner_ids', 'child_of', [partner.commercial_partner_id.id]),
                                              ('installer_id', '=', request.env.user.id)
    ...
    def portal_my_quotes(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        installer = request.env.user.installer
        values.update({
            'date': date_begin,
    ...
    def portal_order_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        if report_type in ('html', 'pdf', 'text'):
            if request.env.user.installer:
                return self._show_report(model=order_sudo, report_type=report_type, report_ref='solai_sale.action_report_saleorder_installer', download=download)
        ...
        if request.env.user.installer:
            return request.render('solai_sale.sale_order_portal_template_installer', values)
        else:
